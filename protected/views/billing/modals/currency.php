...
<div id="currencycreate" class="modal" style="border-radius: 5px;">
<?php
  ...
  <div class="row">
    <div class="row">
      <div class="input-field col s12">
        <input type="text" name="newcurrency" id="newprice" required="required" autofocus>
        <label for="displayname" class="active">Currency Name</label>
      </div>

      <div class="input-field col s12">
        <input type="text" name="abbrev" id="weight_name" required="required" autofocus>
        <label for="displayname" class="active">Acronym/AbbreviationAcr./Abb.</label>
      </div>

      <select  name="countryUsed" required="required" style="position: absolute; width: 100%;">
        <option value="" disabled selected>Country</option>
        <?php
        $counties = TCountry::model()->findAll("status IN ('C','A')");
            if (!empty($counties)) {
                foreach ($counties as $code) {
              ?>
                <option value="<?php echo $code->id; ?>"><?php echo $code->name.' ('.$code->code.')' ?></option>
              <?php
                            }
                 }
              ?>
