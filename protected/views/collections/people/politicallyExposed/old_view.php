...
$pictures = TPersonPhoto::model()->findAll("person='$personid' and status IN ('D','A')"); //getting person picture urls
$picture = TPersonPhoto::model()->find("person='$personid' and status IN ('D','A')"); //getting person picture urls only one
...
$draftphoto = TPersonPhoto::model()->findAll("person='$personid' and status = 'D'"); //getting all newly created photo
...
$persondraftphoto = TPersonPhoto::model()->findAll("person='$personid' and status='D'and maker='$userid'"); //getting only newly user created positions
...
?>
<div class="row search-tabs-row search-tabs-header white">
                    ...
                    <div class="card-content  white-text">
                        <div class="row s12 m12">
                            <div class="col s12 m9">
                                <h4><?php echo $name; ?></h4>
                                <address>
                                    <?php echo $gendername . ', ' . $nationalityname; ?><br>
                                    P: +256 ### ### ###
                                </address>
                            </div>
                            <div class="col s12 m3">
                                <div class="center-align search-image-results">
                                    <img <?php if (!empty($picture) and count($picture) >= 1) { ?>src="<?php echo $picture->url; ?>"<?php } else { ?>src="assets/images/user.png"<?php } ?> class="responsive-img circle center"  alt="Missing Profile Picture" style="width: 50%;height: 50%;" >
                                </div>
                            </div>

                        </div>
                        <div class="center">
                            <?php if (count($citations) >= 1 and $personcount >= 1) { ?><button href="#submit-records" class="modal-trigger btn waves-effect waves-blue btn blue">Submit</button>
                            <?php } else { ?><button class="btn waves-effect waves-black btn cyan darken-4 tooltipped" data-position="top" data-delay="50" data-tooltip="No Newly Added Atribute or Reference">Submit</button><?php } ?>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="card card-transparent no-m">
    <div class="card-content invoice-relative-content search-results-container">
        <div class="col s12 m12 l12">
            <div class="">
                <a href="#edit_person" class="right-align modal-trigger btn-floating btn-large waves-effect waves-light light-cyan invoice-edit-btn tooltipped" data-position="left" data-delay="50" data-tooltip="Edit Person"><i class="material-icons">edit</i></a>            
            </div>
                            ...
                            <div class="card-content">
                                <div class="tabs-vertical">
                                    <ul class="tabs transparent z-depth-0">
                                        <fieldset><legend class="black-text"><i class="material-icons tiny">visibility</i>View</legend>
                                            <li class="tab">
...
//                                                        include 'modal/editposition.php';
                                                    }
                                                } else {
...
//                                            nationality name
                                                                                ...
                                                                                ?>
                                                                                <tr onclick="location.href = '<?php echo Yii::app()->baseUrl; ?>/index.php?r=people/politicallyExposed/addnewemployment&id=<?php echo $code->encode($result->id); ?>&per=<?php echo $code->encode($personid); ?>'" >
                                                                                    <td><?php echo $t ?></td>
            ...
            <?php
            $t++;
//                                                                   include 'modal/addnewemployment.php';
        }
        ?>
                                                                        </tbody>
                                                                    </table></div></div>
